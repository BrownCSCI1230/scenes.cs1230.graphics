...
const TripleInput: React.FC<TripleInputProps> = ({ label, x, y, z, onXChange, onYChange, onZChange }) => {
  ...
  return (
    <div className="flex items-center">
      ...
      <input className="w-16 mr-2" type="number" value={x} onChange={handleXChange} onWheel={handleWheelX}/>
      <input className="w-16 mr-2" type="number" value={y} onChange={handleYChange} onWheel={handleWheelY}/>
      <input className="w-16" type="number" value={z} onChange={handleZChange} onWheel={handleWheelZ}/>
    </div>
  );
};
