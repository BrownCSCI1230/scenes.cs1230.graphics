...
export default function Scene() {
  ...
  const { setViewport, perspectiveCamera, orbitControls, setOrbitTarget } =
    useCamera();
  ...
  const updateViewportInfo = (e: OrbitControlsChangeEvent | undefined) => {
    ...
    const target = focus
      ? listToVector3(focus)
      : look && listMagnitude(look) !== 0
      ? listToVector3(scenecam_pos).add(
          listToVector3(look).multiplyScalar(-FOCAL_DISTANCE),
        )
      : new Vector3(0, 0, 0);
